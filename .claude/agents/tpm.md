...
6. **Optimize for execution cost and speed:**
   - Each task becomes a separate agent invocation (~$0.10–$2.00 each)
   - Consolidate tiny tasks for the same role touching the same module into one medium task
   - Split large cross-cutting tasks into role-specific tasks that can run in parallel
   - Aim for a wide, shallow dependency graph (≤5–7 levels deep) rather than deep chains
   - Tasks at the same dependency level with overlapping files_to_modify get serialized — avoid this
